...
def draw_weather_card(weather_data):
    ...
    Использует стандартный системный шрифт Ubuntu Linux.
    """
    background_path = "ChatGPT Image 14 июл. 2026 г., 21_48_45.png"
    output_path = "bcz_weather_output.png"
    
    # Список системных шрифтов в Linux на выбор (скрипт проверит их по очереди)
    possible_fonts = [
    ...
    # Ищем доступный в системе шрифт
    font_file = None
    ...
    if font_file:
        try:
            print(f"Использую системный шрифт: {font_file}")
            font_date = ImageFont.truetype(font_file, 30)
            font_temp = ImageFont.truetype(font_file, 85)
            font_desc = ImageFont.truetype(font_file, 28)
            font_sub = ImageFont.truetype(font_file, 22)
        except Exception as e:
            print(f"Не удалось загрузить системный шрифт, откат на дефолтный: {e}")
            font_date = font_temp = font_desc = font_sub = ImageFont.load_default()
    else:
        print("Системные шрифты не найдены, использую стандартный Pillow шрифт.")
        font_date = font_temp = font_desc = font_sub = ImageFont.load_default()
    ...
    # Настройки размеров плашки и отступов (под правый нижний угол)
    width, height = img.size
    p_width, p_height = 500, 360
    margin_right, margin_bottom = 60, 100
    ...
    # Отрисовываем красивую темно-синюю плашку с прозрачностью
    draw.rounded_rectangle([x1, y1, x2, y2], radius=25, fill=(15, 23, 42, 180))
    
    # Отрисовка текста поверх плашки
    draw.text((x1 + 35, y1 + 35), date_str.upper(), font=font_date, fill=(255, 255, 255, 255))
    
    # Температура
    sign = "+" if temp_now > 0 else ""
    temp_str = f"{sign}{temp_now}°C"
    draw.text((x1 + 35, y1 + 80), temp_str, font=font_temp, fill=(255, 255, 255, 255))
    
    # Описание статуса погоды
    draw.text((x1 + 35, y1 + 185), f"{status_emoji} {status_text}", font=font_desc, fill=(241, 245, 249, 255))
    
    # Прогноз на день
    sign_min = "+" if temp_min > 0 else ""
    sign_max = "+" if temp_max > 0 else ""
    daily_str = f"Днем: {sign_max}{temp_max}° | Ночью: {sign_min}{temp_min}°"
    draw.text((x1 + 35, y1 + 245), daily_str, font=font_sub, fill=(203, 213, 225, 255))
    
    # Скорость ветра и влажность
    info_str = f"💨 Ветер: {wind} м/с  |  💧 Влажность: {humidity}%"
    draw.text((x1 + 35, y1 + 290), info_str, font=font_sub, fill=(203, 213, 225, 255))
